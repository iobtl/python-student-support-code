        ...
        match t1:
            case TupleType(ts1):
                match t2:
                    case TupleType(
                        ts2
                    ):  # if len(ts1) == len(ts2):  -- including this breaks Llambda type checking because of bogus treatment of closure types there
        ...
        match ss[0]:
            ...
            case Assign([Subscript(tup, Constant(index), Store())], value):
                ...
                match tup_t:
                    ...
                    case _:
                        raise Exception(
                            "type_check_stmts: expected a tuple, not " + repr(tup_t)
                        )
                return self.type_check_stmts(ss[1:], env)
            case _:
