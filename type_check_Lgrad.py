...
class TypeCheckLgrad(TypeCheckLlambda):
    ...
    def consistent(self, t1, t2):
        match (t1, t2):
            ...
            case (FunctionType(ps1, rt1), FunctionType(ps2, rt2)):
                return all(
                    [self.consistent(p1, p2) for (p1, p2) in zip(ps1, ps2)]
    ...
    def join_types(self, t1, t2):
        match (t1, t2):
            ...
            case (FunctionType(ps1, rt1), FunctionType(ps2, rt2)):
                return FunctionType(
                    [self.join_types(p1, p2) for (p1, p2) in zip(ps1, ps2)],
                    self.join_types(rt1, rt2),
                )
            case (TupleType(ts1), TupleType(ts2)):
                return TupleType(
                    [self.join_types(ty1, ty2) for (ty1, ty2) in zip(ts1, ts2)]
                )
            case (ListType(ty1), ListType(ty2)):
    ...
    def type_check_exp(self, e, env):
        match e:
            ...
            case Call(Name("input_int"), []):
                return IntType()
            case BinOp(left, op, right) if isinstance(op, Add) or isinstance(
                op, Sub
            ) or isinstance(op, Mult):
            ...
            case Compare(left, [cmp], [right]) if isinstance(cmp, Eq) or isinstance(
                ...
                return BoolType()
            case Compare(left, [cmp], [right]) if isinstance(cmp, Lt) or isinstance(
                cmp, LtE
            ...
            case Subscript(tup, Constant(index), Load()) if isinstance(index, int):
                ...
                match tup_ty:
                    ...
                    case _:
                        raise Exception(
                            "subscript expected a tuple, not " + repr(tup_ty)
                        )
            ...
            case Lambda(params, body):
                ...
                match ty:
                    ...
                    case AnyType():
                        ...
                        for p in new_params:
                            new_env[p] = AnyType()
                        e.has_type = FunctionType(
                            [AnyType() for _ in new_params], AnyType()
                        )
                    case Bottom():
                        pass
                    case _:
        ...
        match s:
            ...
            case Assign([Subscript(tup, index, Store())], value):
                ...
                match tup_t:
                    ...
                    case _:
                        raise Exception(
                            "type_check_stmts: expected a list or tuple, not "
                            + repr(tup_t)
                        )
            # Cases in Lfun
