...
RULES = {
    ...
    "rule": [
        "RULE template_params _COLON expansions _NL",
    ...
    "expansions": ["alias", "expansions _OR alias", "expansions _NL _OR alias"],
    ...
    def template_usage(self, c):
        if c[0] in self.names:
            return self.__default__(
                "template_usage", [self.names[c[0]].name] + c[1:], None
            )
...
def _make_joined_pattern(regexp, flags_set):
    ...
    if not Py36:
        if len(flags_set) > 1:
            raise GrammarError(
                "Lark doesn't support joining terminals with conflicting flags in python <3.6!"
            )
...
class Grammar:
    ...
    def compile(self, start, terminals_to_keep):
        ...
        transformer = (
            PrepareLiterals() * PrepareSymbols() * anon_tokens_transf
        )  # Adds to terminals
                ...
                if isinstance(item, Token):
                    ...
                    if item.type == "TERMINAL":
                        try:
                            ...
                            raise GrammarError(
                                "Terminal used but not defined: %s" % item
                            )
...
def options_from_rule(name, params, *x):
    if len(x) > 1:
        ...
        priority = None
    params = (
        [t.value for t in params.children] if params is not None else []
...
def symbols_from_strcase(expansion):
    return [
        Terminal(x, filter_out=x.startswith("_")) if x.isupper() else NonTerminal(x)
        for x in expansion
    ]
...
def _get_parser():
    try:
        ...
        rules = [
            Rule(NonTerminal(r), symbols_from_strcase(x.split()), i, None, o)
            for r, _p, xs, o in rules
...
class GrammarBuilder:
    def __init__(
        self, global_keep_all_tokens=False, import_paths=None, used_files=None
    ):
    ...
    def _extend(self, name, exp, params=(), options=None):
        if name not in self._definitions:
            self._grammar_error(
                "Can't extend {type} {name} as it wasn't defined before", name
            )
        if tuple(params) != tuple(self._definitions[name][0]):
            self._grammar_error(
                "Cannot extend {type} with different parameters: {name}", name
            )
        ...
        while len(base.children) == 2:
            assert (
                isinstance(base.children[0], Tree)
                and base.children[0].data == "expansions"
            ), base
