        ...
        match e:
            ...
            case Call(Name("is_tuple_proxy"), [arg]):
                ...
                match arg_ty:
                    ...
                    case _:
                        raise Exception(
                            "type_check error: expected ProxyOrTupleType, not "
                            + repr(arg_ty)
                            + " in "
                            + repr(e)
                        )
            case Call(Name("is_array_proxy"), [arg]):
                ...
                match arg_ty:
                    ...
                    case _:
                        raise Exception(
                            "type_check error: expected ProxyOrListType, not "
                            + repr(arg_ty)
                        )
            ...
            case Call(Name("proxy_array_load"), [lst, index]):
                ...
                match lst_ty:
                    ...
                    case _:
                        raise Exception(
                            "proxy_array_load expected a proxy-or-list, not "
                            + repr(lst_ty)
                        )
            case Call(Name("proxy_array_store"), [tup, index, value]):
                ...
                match tup_t:
                    ...
                    case _:
                        raise Exception(
                            "type_check_exp: in proxy_array_store unexpected "
                            + repr(tup_t)
                        )
