    ...
    def parse_type_annot(self, annot):
        match annot:
            ...
            case Subscript(Name("Callable"), Tuple([ps, rt])):
                return FunctionType(
                    [self.parse_type_annot(t) for t in ps.elts],
        ...
        match ss[0]:
            case FunctionDef(name, params, body, dl, returns, comment):
                ...
                if len(unique_names) != len(new_params):
                    raise Exception(
                        "type_check: duplicate parameter name in function " + repr(name)
                    )
                ...
                for s in body:
                    match s:
                        case FunctionDef(name, params, bod, dl, returns, comment):
                            ...
                            if name in env:
                                raise Exception(
                                    "type_check: duplicate function name " + name
                                )
                            env[name] = FunctionType(
