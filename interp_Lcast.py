        ...
        match (source, target):
            case (AnyType(), FunctionType(ps2, rt2)):
                ...
                return self.apply_cast(
                    self.apply_project(value, anyfun), anyfun, target
                )
            case (AnyType(), TupleType(ts2)):
                anytup = TupleType([AnyType() for t1 in ts2])
                return self.apply_cast(
                    self.apply_project(value, anytup), anytup, target
                )
            case (AnyType(), ListType(t2)):
                anylist = ListType([AnyType() for t1 in ts2])
                return self.apply_cast(
                    self.apply_project(value, anylist), anylist, target
                )
            ...
            case (ListType(t1), AnyType()):
                ...
                return self.apply_inject(
                    self.apply_cast(value, source, anylist), anylist
                )
            case (_, AnyType()):
            ...
            case (TupleType(ts1), TupleType(ts2)):
                x = generate_name("x")
                reads = [
                    Function("cast", [x], [Return(Cast(Name(x), t1, t2))], {})
            ...
            case (t1, t2):
                raise Exception(
                    "apply_cast unexpected " + repr(source) + " " + repr(target)
                )
        ...
        match aggregate:
            case ProxiedTuple(tup, reads):
                return self.apply_fun(
                    reads[index], [self.interp_getitem(tup, index)], None
                )
